...
class Ballistic_Model_learning_RBF(Speed_Model_learning_RBF_angle_state):
    """
    GP model for ball free-flight ballistic dynamics (MC-PILOT).

    Augmented state layout (must match ThrowingSystem):
        s = [x, y, z, vx, vy, vz, Px, Py]   (8-D)

    The GP only needs ball state (dims 0:6) to predict velocity changes.
    Target dims (6:8) are constant throughout a throw and are carried
    forward unchanged in get_next_state_from_gp_output().

    GP setup:
        num_gp   = 3          (one per: delta_vx, delta_vy, delta_vz)
        GP input  = [x,y,z, vx,vy,vz]  (6-D, no control input, no target)
        GP output = [delta_vx, delta_vy, delta_vz]
    """

    BALL_STATE_DIM = 6   # dims 0:6
    ...
    def __init__(
        ...
        super(Ballistic_Model_learning_RBF, self).__init__(
            num_gp=num_gp,
            init_dict_list=init_dict_list,
            T_sampling=T_sampling,
            angle_indeces=[],
    ...
    def data_to_gp_input(self, states, inputs):
        """GP input = ball state only [x,y,z, vx,vy,vz] — strips target and control input."""
        return states[:, 0:self.BALL_STATE_DIM]
    ...
    def get_next_state_from_gp_output(
        ...
        """Integrate velocity changes; carry target dims (6:8) forward unchanged."""
        delta_vel_mean = torch.cat(gp_output_mean_list, 1)
        delta_vel_var  = torch.cat(gp_output_var_list,  1)
        ...
        next_states[:, self.BALL_STATE_DIM:] = current_state[:, self.BALL_STATE_DIM:]
