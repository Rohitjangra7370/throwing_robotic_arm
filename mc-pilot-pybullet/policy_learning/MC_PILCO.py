...
class MC_PILOT(MC_PILCO):
    """
    MC-PILOT: MC-PILCO adapted for the Pick-and-Throw task.

    Key differences from vanilla MC-PILCO
    ----------------------------------------
    1. The simulation system is ThrowingSystem (not an ODE model).
       It accepts an augmented state s = [x,y,z, vx,vy,vz, Px,Py] and
       calls the policy ONCE at t=0 to get a release speed.

    2. Each data-collection episode uses a different random target P.
       A target_sampler callable is provided at construction time.

    3. apply_policy() is overridden to give each of the M=400 particles
       a different random target drawn from the target distribution,
       so the policy learns to generalise across the full target space
       in a single optimisation step.

    Parameters
    ----------
    target_sampler : callable() -> np.ndarray [target_dim]
        Returns a random target position every time it is called.
        For ground-plane 2D: returns (Px, Py).
    release_position : np.ndarray [3]
        Fixed release point in 3-D space, e.g. [0, 0, 0.5].
    throwing_system : ThrowingSystem instance
        Pre-constructed simulator.  Passed instead of f_sim/f_model so
        the caller can configure mass, drag, wind etc. before handing it over.
    All other __init__ kwargs are forwarded to MC_PILCO.__init__.
    """
    ...
    def __init__(
        ...
        dummy_ode = lambda y, t, u: np.zeros_like(y)
        super(MC_PILOT, self).__init__(
            T_sampling=T_sampling,
            state_dim=state_dim,
            input_dim=input_dim,
            f_sim=dummy_ode,
        ...
        # Replace ODE system with the throwing simulator
        self.system = throwing_system
    ...
    def _make_augmented_s0(self, target=None):
        ...
        if target is None:
            target = self.target_sampler()
        return np.concatenate([
            self.release_position,          # [x, y, z]
            np.zeros(3),                    # [vx, vy, vz] = 0 (policy sets these)
            np.array(target, dtype=float),  # [Px, Py]
        ])

    def get_data_from_system(self, initial_state, T_exploration, trial_index, flg_exploration=False):
        """
        Override: sample random target, build augmented s0, then run the
        ThrowingSystem rollout.  The initial_state argument is ignored —
        it is only kept to preserve the parent method signature.
        """
        if flg_exploration:
            current_policy = self.rand_exploration_policy
        else:
    ...
    def apply_policy(
        ...
        """
        Override: embed the policy's release speed into the initial particle
        velocity so that gradients flow from landing cost → GP chain →
        initial velocity → speed → policy parameters.

        Gradient chain:
          cost ← p_final ← GP propagation ← states[0][:,3:6] (v3d)
               ← speed*cos/sin ← speed = policy(P) ← policy params  ✓

        Each particle gets a different random target P so the policy
        optimises over the full target distribution in one gradient step.
        """
        states_sequence_list = []
        inputs_sequence_list = []
        ...
        # --- arm-side noise: policy commands v_cmd; arm delivers perturbed velocity ---
        # scale and additive are detached constants; gradient flows through v3d only.
        # VelocityBiasNoise:    v3d_actual = v3d + dv          (scale=1, additive=N)
        # VelocitySlipNoise:    v3d_actual = (1-α)*v3d + dv    (scale=1-α, additive=N)
        # ReleaseTimingJitter:  v3d_actual = scale(t_d)*v3d    (scale<1, additive=0)
        if self.arm_noise is not None:
            v3d_np = v3d.detach().cpu().numpy()                                  # [M, 3]
            scale_np, additive_np = self.arm_noise.perturb_numpy(v3d_np, num_particles)
            scale    = torch.tensor(scale_np[:, None], dtype=self.dtype,
                                    device=self.device)                          # [M, 1]
            additive = torch.tensor(additive_np, dtype=self.dtype,
                                    device=self.device)                          # [M, 3]
            v3d = scale * v3d + additive                                         # grad via v3d

        # --- initial particle state: release position + release velocity + target ---
        init_particles = torch.cat([ball_pos, v3d, targets], dim=1)             # [M, 8]
        states_sequence_list.append(init_particles)
        inputs_sequence_list.append(speed)   # [M, 1] — for logging only

        # --- GP propagation for t=1..T (no control input during free flight) ---
        # Particles that have landed (z <= 0) are frozen at their landing position,
        # as per the MC-PILOT paper Section 4.2.2: "if the particle reaches z_p or
        # below at time step t'_c, then x̃_{t'+1} = x̃_{t'} for t' >= t'_c".
        zero_input = torch.zeros(num_particles, 1, dtype=self.dtype, device=self.device)
